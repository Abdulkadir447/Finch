...
def export_report(report: ReportData, fmt: str) -> Tuple[bytes, str, str]:
    ...
    safe_title = "".join(c if c.isalnum() or c in " -" else "" for c in report.title).strip().replace(" ", "_") or "report"
    ...
    if fmt == "xlsx":
        return render_xlsx(report), f"{base}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    if fmt == "pdf":
